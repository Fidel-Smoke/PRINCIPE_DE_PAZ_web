...
export default function Crud() {
  ...
  const handleSubmit = async e => {
    ...
    // Extraer grado: si es "1001", "1002", etc., toma los primeros 2 dígitos como grado
    let grado = 'TR';
    ...
    // Aplica descuento de pensión (sea docente o descuento manual)
    let pensionFinal = form.es_docente ? costosBase.pension / 2 : costosBase.pension;
    pensionFinal = pensionFinal - (pensionFinal * (parseFloat(form.descuento_pension || 0) / 100));

    // Carnet, agenda y seguro editables
    ...
    const totalPagado = costosBase.matricula + pensionFinal * form.meses_pagados.length + valor_carne + valor_agenda + valor_seguro;
    ...
    const data = {
      ...
      valor_pension: Math.round(pensionFinal),
      valor_carne,
      valor_agenda,
      valor_seguro,
      total_pagado: Math.round(totalPagado),
      valor_esperado: Math.round(valorEsperado),
      deuda: Math.round(valorEsperado - totalPagado),
      meses_pagados: JSON.stringify(form.meses_pagados)
    };

    const repetido = estudiantes.find(est => est.documento_estudiante === form.documento_estudiante && est.id !== form.id);
    if (repetido) {
    ...
    setForm({
      nombre_estudiante: '', documento_estudiante: '', curso: '', nombre_acudiente: '', documento_acudiente: '', observaciones: '', referencia_pago: '', meses_pagados: [], es_docente: false, descuento_pension: 0, incluye_carne: true, incluye_agenda: true, incluye_seguro: true
    });
    cargarEstudiantes();
  };
  ...
  const editar = est => {
    setForm({
      ...
      descuento_pension: est.descuento_pension || 0,
      incluye_carne: !!est.incluye_carne,
      incluye_agenda: !!est.incluye_agenda,
      incluye_seguro: !!est.incluye_seguro
    });
  };
  ...
  return (<div className="container py-4">
    ...
    <form onSubmit={handleSubmit} className="card p-4 shadow-sm mb-5">
      <div className="row g-3">
        {[['nombre_estudiante', 'Nombre estudiante'], ['documento_estudiante', 'Documento'], ['curso', 'Curso'], ['nombre_acudiente', 'Nombre acudiente'], ['documento_acudiente', 'Documento acudiente'], ['referencia_pago', 'Referencia pago'], ['observaciones', 'Observaciones']].map(([name, placeholder]) => (
        ...
        <div className="col-md-4">
          ...
          <label className="form-check">
            <input className="form-check-input" type="checkbox" name="incluye_carne" checked={form.incluye_carne} onChange={handleChange} /> Incluir Carné
          </label>
          <label className="form-check">
